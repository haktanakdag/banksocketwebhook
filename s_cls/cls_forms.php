...
class clsForms {
	public function doGrid ($basliklar,$degerler,$veriler,$dumeler="",$style){
		...
		$baslik="";
		$sonuc="";
		$baslik = "<table border ='1' class='table table-striped table-sm' width='100%'><tr>";
		foreach($basliklar as $b){
			$kolonlar.="<th width='$b[1]'>$b[0]</th>";
		}
		if ($dumeler){$kolonlar.= "<th scope='col' width='20%'>Opsiyon</th>";}
		$baslik .=$kolonlar."</tr>";
		echo $baslik;
			...
			foreach ($veriler as $veri){
				...
				if ($dumeler<>""){
					...
					//if($dume[0]=="duzenle" or $dume[0]=="yeniekle" ) $onclick = str_replace("/", '"',"onclick=/return hs.htmlExpand(this, { objectType: 'ajax' } )/");
						echo "<a href='$dume[1]&$dume[0]=$veri->id' class='btn btn-primary' $onclick> $dume[2] </a>";
					}
					echo "</td>";
				}
				echo "</tr>";
	...
	$deger= ceil($kayitsayisi/$satirsayisi);
	  ...
	  for($i=1;$i<=$deger;$i++){
		echo "<li><a class='page-link' href='$sayfaadi&sayfano=$i'>$i</a></li>";
	 }
        echo "</ul>";
	...
	echo"<input type='text' name='$ozellikler[0]' id='$ozellikler[0]' value='$ozellikler[1]' class ='form-control' $reg>";
	}
	...
	public function doInputP($ozellikler)
        {
        if($ozellikler[2]=='req'){ $reg ='required';}
	echo"<input type='password' name='$ozellikler[0]' id='$ozellikler[0]' value='$ozellikler[1]' class ='form-control' $reg>";
        }
	...
	public function doButton($ozellikler){
			echo "<input type='submit' name='$ozellikler[0]' id='$ozellikler[0]' value='$ozellikler[1]' class='btn btn-primary'>";
	}
	...
	echo "<a href='$ozellikler[2]' class='btn btn-primary' name='$ozellikler[0]' $target  $onclick >$ozellikler[1]</a>";
	}
	...
	public function doSelect($ozellikler,$data){
            echo "<select name='$ozellikler[0]' id='$ozellikler[0]' class ='form-control'>";
            foreach($data as $d){
            if ($d->id ==$ozellikler[2]){$selected='selected';} else {$selected="";} 		
            echo "<option value='".$d->$ozellikler[1]."' $selected >".$d->$ozellikler[1]."</option>"; 
            }
            echo "<select>";
	}
